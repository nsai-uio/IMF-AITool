...
os.makedirs(app.config['SELF_CHECKED_DATA_FOLDER'], exist_ok=True)
os.makedirs(app.config['CUSTOM_DATA_FOLDER'], exist_ok=True)
os.makedirs(app.config['IMF_DATA_FOLDER'], exist_ok=True)
...
def self_check_task(task_id, json_filename):
    try:
        ...
        processed_json_path = os.path.join(app.config['RELATIONS_FOLDER'], json_filename)
...
def step2_relations():
    ...
    # Normalize filename to base PDF name for lookup
    if filename.endswith('_components.json'):
        base_filename = filename.replace('_components.json', '.pdf')
    elif filename.endswith('.json'):
        base_filename = filename.replace('.json', '.pdf')
    else:
...
def list_exports(base_filename):
    files_to_check = {
        ...
        'log_file': ('logs', f"{base_filename}_self_check.log"),
        'checked_file': ('self_checked_data', f"{base_filename}_checked.json"),
    }
    
    available_files = {}
    
    for key, (folder_key, filename) in files_to_check.items():
        folder_path = app.config[folder_key.upper() + '_FOLDER']
...
def export_custom_relations():
    ...
    if not base_filename or not relations_param:
        return jsonify({'error': 'Missing parameters'}), 400
        
    relations_to_keep = relations_param.split(',')
    
    # Path to checked file
    checked_filename = f"{base_filename}_checked.json"
    file_path = os.path.join(app.config['SELF_CHECKED_DATA_FOLDER'], checked_filename)
    
    if not os.path.exists(file_path):
        return jsonify({'error': 'Self-checked file not found. Please run self-verification first.'}), 404
    ...
    try:
        ...
        for comp, details in data.items():
            # Always keep tagID, plus selected relations
            filtered_details = {'tagID': details.get('tagID', '')}
            for rel in relations_to_keep:
                if rel in details:
                    filtered_details[rel] = details[rel]
            filtered_data[comp] = filtered_details
            
        # Create response
        mem = BytesIO()
        mem.write(json.dumps(filtered_data, indent=4).encode('utf-8'))
        mem.seek(0)
        
        return send_file(
            mem,
            as_attachment=True,
            download_name=f"{base_filename}_custom.json",
...
def visualize_custom_relations():
    ...
    try:
        ...
        custom_filename = f"{base_filename}_custom.json"
        custom_path = os.path.join(app.config['CUSTOM_DATA_FOLDER'], custom_filename)
        with open(custom_path, 'w') as f:
            json.dump(filtered_data, f, indent=4)
            
        return jsonify({'filename': custom_filename})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def convert_to_imf():
    ...
    if not base_filename or not model_type:
        return jsonify({'error': 'Missing parameters'}), 400

    input_folder = ''
    input_filename = ''
    
    if model_type == 'preliminary':
        input_folder = app.config['RELATIONS_FOLDER']
        input_filename = f"{base_filename}.json"
    elif model_type == 'verified':
        input_folder = app.config['SELF_CHECKED_DATA_FOLDER']
        input_filename = f"{base_filename}_checked.json"
    elif model_type == 'custom':
        input_folder = app.config['CUSTOM_DATA_FOLDER']
        input_filename = f"{base_filename}_custom.json"
    else:
        return jsonify({'error': 'Invalid model type'}), 400
        
    input_path = os.path.join(input_folder, input_filename)
    if not os.path.exists(input_path):
        return jsonify({'error': f'Source file for {model_type} model not found.'}), 404
        
    output_filename = f"{base_filename}_{model_type}.imf"
    output_path = os.path.join(app.config['IMF_DATA_FOLDER'], output_filename)
    
    try:
        convert_json_to_imf(input_path, output_path)
        return send_from_directory(app.config['IMF_DATA_FOLDER'], output_filename, as_attachment=True)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def download_file(folder, filename):
    folder_map = {
        ...
        'logs': app.config['LOGS_FOLDER'],
        'self_checked_data': app.config['SELF_CHECKED_DATA_FOLDER']
    }
    ...
    if not os.path.exists(file_path):
        return jsonify({'error': f'File not found in {folder}.'}), 404
...
def chat():
    ...
    # 1. Load IMF Manual Context (Always included if available)
    imf_filename = "IMF-Manual-v0.4.0.pdf"
    imf_path = os.path.join(app.config['UPLOAD_FOLDER'], imf_filename)
    if os.path.exists(imf_path):
        try:
            reader = PdfReader(imf_path)
            imf_text = ""
            for page in reader.pages:
                imf_text += page.extract_text() or ""
            context_text += f"IMF Manual ({imf_filename}):\n\"\"\"\n{imf_text}\n\"\"\"\n\n"
        except Exception as e:
            print(f"Error reading IMF manual: {e}")

    # 2. Load User File Context (if selected)
    if filename:
        ...
        if os.path.exists(pdf_path):
            try:
                ...
                for page in reader.pages:
                    pdf_text += page.extract_text() or ""
                context_text += f"User Document ({pdf_filename}):\n\"\"\"\n{pdf_text}\n\"\"\"\n\n"
            except Exception as e:
...
def get_processed_data(filename):
    ...
    if os.path.exists(relations_path):
        return send_from_directory(app.config['RELATIONS_FOLDER'], secure_filename(filename))
    
    # Check components folder
    components_path = os.path.join(app.config['COMPONENTS_FOLDER'], secure_filename(filename))
    if os.path.exists(components_path):
        return send_from_directory(app.config['COMPONENTS_FOLDER'], secure_filename(filename))

    # Check self_checked_data folder
    checked_path = os.path.join(app.config['SELF_CHECKED_DATA_FOLDER'], secure_filename(filename))
    if os.path.exists(checked_path):
        return send_from_directory(app.config['SELF_CHECKED_DATA_FOLDER'], secure_filename(filename))

    # Check custom_data folder
    custom_path = os.path.join(app.config['CUSTOM_DATA_FOLDER'], secure_filename(filename))
    if os.path.exists(custom_path):
        return send_from_directory(app.config['CUSTOM_DATA_FOLDER'], secure_filename(filename))
